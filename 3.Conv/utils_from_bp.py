    ...
    def __init__(self, use_conv = True): # 可以把Network理解成一个大矩阵。这里num_input就是num_feature 784
        ...
        if use_conv:
            self.backbone = Sequential(
                ...
                Conv2d(in_feature = 1, out_feature = 5, ksize= 3),
                ReLU(),
                Flatten(),
...
if __name__ == "__main__":
    # 1. forward check
    itensor = np.random.uniform(size = (10,3,4,4)) 
    wconv = naive_Conv2d(3,2,2)
    fconv = Conv2d(3,2,2)

    wout = wconv(itensor)
    fout = fconv(itensor)
    
    if np.allclose(wout, fout):
        print(f"in_shape:  {itensor.shape}")           # b, c, h,  w
        print(f"w_shape:   {wconv.kernel.data.shape}") # o, c, kh, kw
        print(f"out_shape: {wout.shape}")              # b, o, oh, ow

        G = fout
        # 2. backward check
        FGout = fconv.backward(G)
        WGout = wconv.backward(G)
        print(f"FGout.shape: {FGout.shape}")
        print(f"WGout.shape: {WGout.shape}")
        
        print(np.allclose(FGout, WGout))
