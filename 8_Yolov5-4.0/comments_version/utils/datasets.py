...
from utils.general import xyxy2cxcywh, cxcywh2xyxy, torch_distributed_zero_first
...
def create_dataloader(path, imgsz, batch_size, stride, opt, hyp=None, augment=False, cache=False, pad=0.0, rect=False,
    ...
    """
    # Make sure only the first process in DDP(DistributedDataParallel) process the dataset first,  #todo what is it?
    # and the following others can use the cache.
    with torch_distributed_zero_first(rank): #todo
    ...
    def __getitem__(self, index):
        ...
        if mosaic:
            ...
            if x.size > 0:
                ...
                # 根据pad调整框的标签坐标，并从归一化的cxcywh->未归一化的xyxy
                labels = x.copy()
