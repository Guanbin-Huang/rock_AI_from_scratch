...
def compute_loss(p, targets, model):  # predictions, targets, model
    ...
    Args:
        ...
        targets: targets.shape = (nt, 6) , 6=iccxcywh,i=0表示第一张图片，c为类别，然后为坐标cxcywh
        model: 模型
...
def build_targets(p, targets, model):
    ...
    Args:
        ...
        targets: GT框；targets.shape = (nt, 6) , 6=iccxcywh,i表示第i+1张图片，c为类别，然后为坐标cxcywh
        model: 模型
    ...
    # 6的第0维度表示当前是第几张图片的label =index，后面是classid cxcywh
    ...
    for i in range(det.nl): # 三个尺度的预测特征图输出分支
        ...
        # 将标签框的cxcywh从基于0~1映射到基于特征图；targets的cxcywh本身是归一化尺度，故需要变成特征图尺度
        t = targets * gain
        # 对每个输出层单独匹配
