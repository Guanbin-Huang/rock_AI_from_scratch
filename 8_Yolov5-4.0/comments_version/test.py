...
from utils.general import (
    ...
    xyxy2cxcywh, clip_coords, plot_images, cxcywh2xyxy, box_iou, output_to_target, ap_per_class, set_logging)
        ...
        for si, pred in enumerate(output):
            ...
            if save_txt:
                ...
                for *xyxy, conf, cls in x:
                    # xyxy格式->cxcywh, 并对坐标进行归一化处理
                    cxcywh = (xyxy2cxcywh(torch.tensor(xyxy).view(1, 4)) / gn).view(-1).tolist()  # normalized cxcywh
                    line = (cls, conf, *cxcywh) if save_conf else (cls, *cxcywh)  # label format
                    # 保存预测类别和坐标到txt文件
            ...
            if save_json:
                ...
                # 转换为cxcywh格式
                box = xyxy2cxcywh(box)  # cxcywh
                
                """
                注意：之前所说的xyxy格式为左上角右下角的坐标，cxcywh是中心点坐标和宽高
                而coco的json格式中的框坐标格式为cxcywh,此处的xy为左上角坐标
            ...
            if nl:
                ...
                # target boxes
                # 获得xyxy格式的框并乘以wh
                tbox = cxcywh2xyxy(labels[:, 1:5]) * whwh
