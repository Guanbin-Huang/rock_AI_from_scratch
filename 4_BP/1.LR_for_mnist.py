...
class MNIST_Dataset(Dataset):
    # 针对mnist数据的解析、加载、预处理(e.g. /255 - 0.5), 加一个全是1的维度 etc
...
for epoch in range(num_epochs):
    for images, labels, onehot_labels in train_loader:
        ...
        # break
        predict = images @ W + bias
        probability = softmax(predict)

        loss = -np.sum(onehot_labels * np.log(probability)) / batch_size

        G = (probability - onehot_labels) / batch_size

        # matrix multiplication
        del_W = images.T @ G
        del_b = np.sum(G, axis = 0, keepdims= True)                        # 算 b 更新量
    ...
    # 每一个epoch 验证一次
    # 方法1： 用test_loader  注意：test_batchsize 一定要能够被10000整除，否则会漏几张没被测
    correct = 0
    for test_images, test_labels, test_one_hot_labels in test_loader:
        predict = test_images @ W + bias
